...
protected[validador] trait Rotina {
  ...
  def validarFromFileParalelo[A](file: File, dataCompetencia: Date, unidadeGestoraArquivo: String, controleArquivo: ControleArquivo, gerarTuplaProcessadorEtapas: (MetaDados) => Try[(Conversor[A], Seq[Etapa])]): Try[ResultadosValidacao[A]] = {
    Processadores.processarLinhasParalelo(Source.fromFile(file, "UTF-8").getLines().toVector.zipWithIndex, MetaDados("", 0, dataCompetencia, unidadeGestoraArquivo, controleArquivo), gerarTuplaProcessadorEtapas)
  }
}